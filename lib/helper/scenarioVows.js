...
exports.testCase = function(resource, typeScenario, callBack) {
    var scenario,
        callback,
        index,
    ...
    typesScenarios.push(typeScenario);
    scenarios = generate.scenariosToUrl(resource, typesScenarios);
    context['topic'] = scenarios;

    if (!scenarios) {
        throw new Error('There is no scenario to ' + typesScenarios.join(', '));
    }
    ...
    function contextFunction(data, response, error, scenario) {
        if (!response && error) {
            ...
            callBack(error, data, response, scenario);
        }
    }
