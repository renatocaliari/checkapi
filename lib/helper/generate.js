...
function generateScenario(config, resource, typeScenario) {
    var fields, nextVariation = undefined, scenarios = [], maxVariation,
        idxScenario = 0,
        contexts = [], variation, variations = {}, bodyScenariosCreated = false;
        fields = orderFields(typeScenario, getFieldsFromResource(config, resource)); 
        maxVariation = fields[0].scenario[typeScenario].length;
        
        scenarios = _.reduce(fields, function(variations, field) {
            idxScenario = 0;
            nextVariation = getVariation(field, typeScenario);
            _(maxVariation).times(function() {
                if (typeScenario === "negative") {
                    typeScenario = "positive";
                };
                variation = nextVariation();
                if (!bodyScenariosCreated) {
                    variations.push(buildScenario(variation.statusHttp, variation.message));
                    _.last(variations).params.push({name: field.name, value: getDynamicValue(variation.value)});
                } else {
                    variations[idxScenario].params.push({name: field.name, value: getDynamicValue(variation.value)});
                }
                idxScenario++;
            });
            bodyScenariosCreated = true;
